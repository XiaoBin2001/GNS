...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, debug_from, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if opt.prune_reg and opt.e1 > iteration > opt.s1 and (iteration-1) % opt.reg_interval == 0:
            if not opacity_min:
                ...
                opacity_goal = (1-(iteration-opt.s1) / (opt.e1-opt.s1)) * opacity_min
                if opacity_goal < 0:
                    opacity_goal = 0
                sorted_values, _ = torch.sort(gaussians.get_opacity.flatten())
                value = sorted_values[gaussians.get_opacity.shape[0] - opt.final_budget].item()
        ...
        elif opt.prune_reg and opt.e2 > iteration > opt.s2 and (iteration-1) % opt.reg_interval == 0:
            if not opacity_min:
                sorted_values, _ = torch.sort(gaussians.get_opacity.flatten())
                value = sorted_values[gaussians.get_opacity.shape[0] - opt.final_budget].item()
                opacity_min = value * 0.8
            elif (iteration-1) % 100 == 0:
                opacity_goal = (1-(iteration-opt.s2) / (opt.e2-opt.s2)) * opacity_min
                if opacity_goal < 0:
                    opacity_goal = 0
                sorted_values, _ = torch.sort(gaussians.get_opacity.flatten())
                value = sorted_values[gaussians.get_opacity.shape[0] - opt.final_budget].item()
        ...
        with torch.no_grad():
            ...
            if 500 < iteration < opt.s1:
            ...
            elif opt.s1 <= iteration < opt.e1:
                if iteration == opt.s1:
                    gaussians.update_opacity_lr(4)
                    opt.prune_reg = True
                if iteration % opt.reg_interval == 0 and opt.prune_reg:
                    gaussians.only_prune(0.01)
                if (iteration == opt.e1 or gaussians.get_opacity.shape[0] < opt.final_budget * 1.05) and opt.prune_reg:
                    gaussians.final_prune(opt.final_budget)
                    opt.prune_reg = False
                    prune_iter = iteration
            elif opt.e1 <= iteration < opt.s2:
                gaussians.add_densification_stats_abs(viewspace_point_tensor, visibility_filter)
                if iteration % 100 == 0:
                    num_cams = args.cams
                    edge_losses = []
                    ...
                    for _ in range(num_cams):
                        ...
                        edge_losses.append(edges_stack.pop())
                    gaussian_importance = compute_edge_score(camlist, edge_losses, gaussians, pipe, bg)
                    rate = (iteration - opt.e1 + 300) / (opt.s2 - opt.e1)
                    if rate >= 1:
                        budget = opt.final_budget * 2
                    else:
                        budget = int(math.sqrt(rate) * opt.final_budget * 2)
                ...
                if iteration % opt.reg_interval == 0 and opt.prune_reg:
                    gaussians.only_prune(0.01)
                if (iteration == opt.e2 or gaussians.get_opacity.shape[0] < opt.final_budget * 1.05) and opt.prune_reg:
                    gaussians.final_prune(opt.final_budget)
                    opt.prune_reg = False
                    prune_iter = iteration

            if iteration == prune_iter + 500:
            ...
            if iteration < opt.iterations:
                if opt.optimizer_type == "default":
                    ...
                    elif iteration <= opt.e1:
                        gaussians.optimizer.step()
                        gaussians.optimizer.zero_grad(set_to_none=True)
                        if iteration % 5 == 0:
                            gaussians.shoptimizer.step()
                            gaussians.shoptimizer.zero_grad(set_to_none=True)
                    elif iteration <= opt.e2:
                        if iteration % 10 == 0:
                            gaussians.optimizer.step()
                            gaussians.optimizer.zero_grad(set_to_none=True)
                            gaussians.shoptimizer.step()
            ...
            if iteration in saving_iterations:
                ...
                print("[ITER {}] TrainingTime {:.0f} seconds\n".format(iteration, train_time_accumulated-test_time_accumulated))
...
def compute_render_score(camlist, edge_losses, gaussians, pipe, bg):
    num_points = len(gaussians.get_xyz)
    gaussian_importance = torch.zeros(num_points, device="cuda", dtype=torch.float32)
    visibility_filter_all = torch.zeros(num_points, device="cuda", dtype=bool)
    for view in range(len(camlist)):
        ...
        loss_accum = normalize(render_pkg["accum_blend"])
        visibility_filter = render_pkg["visibility_filter"].detach()
